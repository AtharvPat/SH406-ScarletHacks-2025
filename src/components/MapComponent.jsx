...
// Helper: generate a random "amoeba" polygon given a center and radius (in meters)
const generateRandomAmoeba = (center, radius, numPoints = Math.floor(Math.random() * 8) + 8) => {
  const points = [];
  for (let i = 0; i < numPoints; i++) {
    const angle = (2 * Math.PI * i) / numPoints;
    // Random factor to vary the radius for each vertex
    const randomFactor = 0.7 + 0.6 * Math.random();
    const randomRadius = radius * randomFactor;
    // Convert meter offset into degrees approximately
    const deltaLat = randomRadius * Math.sin(angle) / 111320;
    const deltaLng = randomRadius * Math.cos(angle) / (111320 * Math.cos(center.lat * Math.PI / 180));
...
const MapComponent = () => {
  ...
  // State for the random polygon paths (amoeba shape)
  const [amoebaPaths, setAmoebaPaths] = useState([]);
  ...
  // Helper: calculate sine and cosine features based on month
  ...
  // Update amoeba shape when markerPosition or mapZoom changes
  useEffect(() => {
    if (markerPosition) {
      // Calculate meters per pixel (approximation)
      const metersPerPixel =
        156543.03392 * Math.cos(markerPosition.lat * Math.PI / 180) /
        Math.pow(2, mapZoom);
      // Use 100 pixels as the base size
      const dynamicRadius = 100 * metersPerPixel;
      const newPaths = generateRandomAmoeba(markerPosition, dynamicRadius);
            ...
            >
              ...
              <div style={{ marginTop: "30px" }}>
                ...
                {matchedData.install_size_kw_buckets_json && (() => {
                  let buckets = [];
                  try {
                    buckets = JSON.parse(matchedData.install_size_kw_buckets_json);
                  } catch (e) {
